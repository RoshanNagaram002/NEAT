from Info import Randomized_Info, Sorted_Randomized_Info, ListDict, Network
import numpy as np
...
class Genome:
    ...
    def _extract_network(self) -> Network:
        nodes = [node.node_id for node in self.nodes]
        edges = [(con.left, con.right, con.weight, con.is_enabled) for con in self.connections]
        return Network(nodes, edges, self.num_input_nodes, self.num_output_nodes)   
